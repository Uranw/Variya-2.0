...
export default function HomeScreen() {
  ...
  const ws = new WebSocket("ws://192.168.16.104:8080");
    ...
    (async () => {
      ...
      await GeoLocation.watchPositionAsync(
        ...
        async (position) => {
          ...
          if (
            !lastLocation ||
            haversineDistance(lastLocation, newLocation) > 300
          ) {
  ...
  const sendPushNotification = async (expoPushToken: string, data: any) => {
    const message = {
      to: expoPushToken,
      sound: "default",
      title: "Variya Request Accepted!",
      body: `Your Variya is on the way!`,
      data: { orderData: data },
    };
    await axios
  ...
  const acceptRideHandler = async () => {
    ...
    await axios
      ...
      .then(async (res) => {
        ...
        const driverPushToken = "ExponentPushToken[ioOkRoEOQUvqyBQrZj1zH7]";
        ...
        <View style={[styles.rideContainer, { backgroundColor: colors.card }]}>
          ...
          <ScrollView>
            ...
            {recentRides?.length === 0 && (
              <Text>You didn't take any Variya yet!</Text>
            )}
          </ScrollView>
        </View>
      </View>
      ...
      >
        <TouchableOpacity style={styles.modalBackground} activeOpacity={1}>
          <TouchableOpacity style={styles.modalContainer} activeOpacity={1}>
            <View>
              <Text style={styles.modalTitle}>New Variya Request Received!</Text>
              <MapView
              ...
              >
                Amount:
                {(distance * parseInt(driver?.rate!)).toFixed(2)} NRS
              </Text>
              <View
                style={{
                  flexDirection: "row",
