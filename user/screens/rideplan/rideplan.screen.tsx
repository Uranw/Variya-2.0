...
export default function RidePlanScreen() {
  ...
  const initializeWebSocket = () => {
    ws.current = new WebSocket("ws://192.168.16.104:8080");
    ws.current.onopen = () => {
  ...
  const sendPushNotification = async (expoPushToken: string, data: any) => {
    const message = {
      to: expoPushToken,
      sound: "default",
      title: "New Variya Request",
      body: "You have a new Variya request.",
      data: { orderData: data },
    };
  ...
  const handleOrder = async () => {
    ...
    const driverPushToken = "ExponentPushToken[EhHXU_H1p8ixUEJ999P0VG]";
